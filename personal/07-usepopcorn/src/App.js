...
function MovieDetails({ id, onCloseMovie, onAddWatched, watched }) {
  ...
  /* eslint-disable*/
  // if (imdbRating > 5) [isTop, setIsTop] = useState(true);
...
//   useEffect(() => {
//     async function fetchAmt() {
//       try {
//         console.log(num, from, to, typeof num, typeof from, typeof to);
//         const res = await fetch(
//           `/latest?amount=${num}&from=${from}&to=${to}`,
//           // `https://api.frankfurter.app/latest?amount=${num}&from=${from}&to=${to}`
//           // `/latest?amount=100&from=EUR&to=USD`,
//         );
//         const data = await res.json();
//         console.log(data);
//         console.log(data.rates[to]);
//         setOp(Number(data.rates[to]));
//       } catch (err) {
//         console.log(err);
